...
namespace Bot.Money.Commands
{
    public class FinanceOperationCommand : IMoneyBotCommand
    {
        ...
        {
            return request.Session.CurrentState != -1 || Regex.IsMatch(request.Message.Text, _floatNumberPattern);
        }
        ...
        public async Task Execute(UserRequest request)
        {
            ...
            if (request.Message.Text is _nameOfCancelButton)
            {
                // _commandSteps.Finish(request.Message.Chat.Id);
                await request.Client.SendTextMessageAsync(chatId: request.Message.Chat, text: "Canceled", replyMarkup: new ReplyKeyboardRemove());
                return;
            }

            request.Session.MoveNext(request.Message);
